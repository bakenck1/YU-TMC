...
export const oneCImportBatchesTable = inventorySchema.table(
  "one_c_import_batches",
  {
    id: uuid().primaryKey(),
    requestId: uuid(),
    sourceFilename: text(),
    sourceSha256: varchar({ length: 64 }).notNull(),
    sourceSnapshotAt: timestamp({ withTimezone: true, mode: "date" }),
    receivedCount: integer().notNull(),
    createdCount: integer().notNull(),
    updatedCount: integer().notNull(),
    unchangedCount: integer().notNull(),
    state: varchar({ length: 32 }).notNull().default("received"),
    receivedAt: timestamp({ withTimezone: true, mode: "date" }).notNull().defaultNow(),
    reviewStartedAt: timestamp({ withTimezone: true, mode: "date" }),
    approvedAt: timestamp({ withTimezone: true, mode: "date" }),
    approvedBy: uuid().references(() => usersTable.id, { onDelete: "restrict", onUpdate: "restrict" }),
    publishedAt: timestamp({ withTimezone: true, mode: "date" }),
    publishedBy: uuid().references(() => usersTable.id, { onDelete: "restrict", onUpdate: "restrict" }),
    summary: jsonb().$type<Record<string, unknown>>().notNull().default(sql`'{}'::jsonb`),
    version: integer().notNull().default(1),
  },
  (table) => [
    unique("one_c_import_batches_source_sha256_unique").on(table.sourceSha256),
    check("one_c_import_batches_hash_check", sql`${table.sourceSha256} ~ '^[0-9a-f]{64}$'`),
    check("one_c_import_batches_counts_check", sql`${table.receivedCount} >= 0 AND ${table.createdCount} >= 0 AND ${table.updatedCount} >= 0 AND ${table.unchangedCount} >= 0 AND ${table.createdCount} + ${table.updatedCount} + ${table.unchangedCount} = ${table.receivedCount}`),
    check("one_c_import_batches_state_check", sql`${table.state} in ('received','analyzing','review_required','approved','publishing','published','failed','rejected','superseded')`),
    check("one_c_import_batches_version_check", sql`${table.version} > 0`),
    index("one_c_import_batches_state_received_idx").on(table.state, table.receivedAt),
  ],
);

export const oneCImportBatchRowsTable = inventorySchema.table(
  "one_c_import_batch_rows",
  {
    batchId: uuid().notNull().references(() => oneCImportBatchesTable.id, { onDelete: "restrict", onUpdate: "restrict" }),
  ...
  (table) => [
    primaryKey({ name: "one_c_import_batch_rows_pk", columns: [table.batchId, table.externalId] }),
    check("one_c_import_batch_rows_hash_check", sql`${table.payloadHash} ~ '^[0-9a-f]{64}$'`),
...
export const itemOneCLinksTable = inventorySchema.table(
  "item_one_c_links",
  {
    ...
    linkedAt: timestamp({ withTimezone: true, mode: "date" }).notNull().defaultNow(),
    linkMethod: varchar({ length: 32 }).notNull(),
    lastBatchId: uuid().notNull().references(() => oneCImportBatchesTable.id, { onDelete: "restrict", onUpdate: "restrict" }),
    lastPayloadHash: varchar({ length: 64 }).notNull(),
    accountingStatus: text(),
    accountingResidualValue: numeric({ precision: 14, scale: 2 }),
    sourceDepartment: text(),
    sourceResponsibleName: text(),
    sourceUpdatedAt: timestamp({ withTimezone: true, mode: "date" }),
    version: integer().notNull().default(1),
  },
  (table) => [
    check("item_one_c_links_method_check", sql`${table.linkMethod} in ('manual','inventory_number_confirmed','created_from_one_c')`),
    check("item_one_c_links_hash_check", sql`${table.lastPayloadHash} ~ '^[0-9a-f]{64}$'`),
    check("item_one_c_links_version_check", sql`${table.version} > 0`),
    index("item_one_c_links_last_batch_idx").on(table.lastBatchId),
  ],
);

export const oneCPublicationRunsTable = inventorySchema.table(
  "one_c_publication_runs",
  {
    id: uuid().primaryKey(),
    batchId: uuid().notNull().references(() => oneCImportBatchesTable.id, { onDelete: "restrict", onUpdate: "restrict" }),
    idempotencyKey: text().notNull().unique(),
    state: varchar({ length: 24 }).notNull(),
    requestedBy: uuid().notNull().references(() => usersTable.id, { onDelete: "restrict", onUpdate: "restrict" }),
    startedAt: timestamp({ withTimezone: true, mode: "date" }).notNull().defaultNow(),
    finishedAt: timestamp({ withTimezone: true, mode: "date" }),
    createdItems: integer().notNull().default(0),
    linkedItems: integer().notNull().default(0),
    updatedItems: integer().notNull().default(0),
    skippedItems: integer().notNull().default(0),
    failedItems: integer().notNull().default(0),
    errorSummary: jsonb().$type<Record<string, unknown>>(),
  },
  (table) => [
    check("one_c_publication_runs_state_check", sql`${table.state} in ('pending','running','completed','failed')`),
    check("one_c_publication_runs_counts_check", sql`${table.createdItems} >= 0 AND ${table.linkedItems} >= 0 AND ${table.updatedItems} >= 0 AND ${table.skippedItems} >= 0 AND ${table.failedItems} >= 0`),
