...
if [[ "${1:-}" != "--container" ]]; then
  if ! command -v docker >/dev/null 2>&1; then
    echo "Docker is required for the isolated deployment smoke test." >&2
    exit 1
  fi
  readonly workspace_directory="${GITHUB_WORKSPACE:-$PWD}"
  exec docker run --rm \
    --env YU_INVENTORY_DEPLOYMENT_CONTAINER=1 \
    --volume "$workspace_directory:/workspace:ro" \
    ubuntu:24.04 \
  bash /workspace/scripts/test-deployment-runtime.sh --container
fi

export DEBIAN_FRONTEND=noninteractive
apt-get update -qq
apt-get install -y -qq nodejs openssl util-linux >/dev/null
...
[[ "${YU_INVENTORY_DEPLOYMENT_CONTAINER:-}" == "1" ]] ||
  fail "the container test entry point may only run inside its isolated Docker container"
...
install -o root -g root -m 0750 \
  /workspace/deploy/backup-postgres.sh \
  /usr/local/sbin/yu-inventory-backup
install -o root -g root -m 0750 \
  /workspace/deploy/backup-postgres-connection.mjs \
...
install -o root -g root -m 0600 \
  /workspace/deploy/nginx/yu-inventory.conf \
  /etc/yu-inventory/yu-inventory.conf
printf '# configuration file /etc/nginx/nginx.conf:\n' >"$test_root/nginx-dump"
/workspace/deploy/enable-https.sh "$certificate_source" >"$test_root/https-success.log"
...
EOF
if /workspace/deploy/enable-https.sh "$certificate_source" >"$test_root/https-conflict.log" 2>&1; then
...
printf '# configuration file /etc/nginx/nginx.conf:\n' >"$test_root/nginx-dump"
if /workspace/deploy/enable-https.sh "$certificate_source" >"$test_root/https-rollback.log" 2>&1; then
