...
Владелец API — команда YU Inventory; потребитель — интеграция Dockflow. API
передаёт персональные данные сотрудника (ИИН, ФИО и рабочие контакты), сведения
об ответственности и стоимости ТМЦ. Эти ответы нельзя кэшировать: JSON и фото
возвращаются с `Cache-Control: private, no-store`, JSON также содержит
`X-Request-Id`. В журналах не должны появляться ключи, ИИН, GUID или содержимое
ответов.

Все коллекции используют `limit` (по умолчанию 100, максимум 200) и непрозрачный
`cursor`; продолжение явно возвращается в `nextCursor`. Порядок ТМЦ стабилен:
`updatedAt DESC, id`; выдачи сотрудника — `assignedAt DESC, id`. Архивные и
списанные ТМЦ, закрытые выдачи, неактивные/удалённые локальные пользователи и
неактивные профили Yessenov ID наружу не попадают. Запрос `Range` для фото
отклоняется как `416 RANGE_NOT_SUPPORTED`.

`cursor` содержит последнюю сортировочную пару и привязан к конкретному виду
коллекции (а для выдач — также к ИИН). Поэтому добавление более новой записи
между страницами не сдвигает продолжение и не создаёт дубликат. Это live-view,
а не снимок: запись, обновлённая после начала обхода и переместившаяся перед
cursor, будет получена при следующем полном цикле синхронизации.

При временной недоступности зависимости клиент получает контролируемый
`502`/`503`; повторять запрос следует не раньше `Retry-After` (если заголовок
присутствует), затем с экспоненциальной задержкой. Целевой таймаут клиента — 10
секунд, не более трёх повторов. Инциденты и отзыв доступа координирует владелец
API.

### Ротация и отзыв ключа

1. Сгенерировать новый случайный ключ в secret store и записать его только в
   `DOCKFLOW_API_KEY_NEXT`; текущий `DOCKFLOW_API_KEY` оставить без изменений.
2. Перезапустить приложение и проверить `/api/v1/auth/check` обоими ключами.
3. Передать новый ключ Dockflow по защищённому каналу и дождаться подтверждения
   переключения.
4. Сделать новый ключ значением `DOCKFLOW_API_KEY`, удалить
   `DOCKFLOW_API_KEY_NEXT`, перезапустить и убедиться, что старый ключ получает
   `401`. Для срочного отзыва этот шаг выполняется сразу.

Ключи хранятся только в secret store окружения; отдельная таблица управления
ключами не создаётся.
