  ...
  return (
    <section className="rounded-[1.75rem] border border-emerald-100 bg-gradient-to-br from-emerald-50 to-white p-5 shadow-sm">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h2 className="font-bold text-zinc-900">{verified ? t("profile.emailVerified") : t("profile.emailUnverified")}</h2>
          <p className="mt-1 text-sm leading-5 text-zinc-500">{verified ? t("profile.emailVerifiedHint") : t("profile.emailUnverifiedHint")}</p>
        </div>
      </div>
    </section>
  );
