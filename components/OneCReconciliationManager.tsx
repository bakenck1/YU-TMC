...
function Card({label,value}:{label:string;value:unknown}){return <div className="rounded-2xl border border-black/5 bg-white p-4"><div className="text-xs text-zinc-500">{label}</div><div className="mt-1 text-xl font-semibold">{String(value??0)}</div></div>;}
function Status({value}:{value:string}){const danger=/conflict|failed|blocked/.test(value);return <span className={`inline-flex rounded-full px-2 py-1 text-xs font-medium ${danger?"bg-red-50 text-red-700":value==="published"?"bg-emerald-50 text-emerald-700":"bg-blue-50 text-blue-700"}`}>{value}</span>;}
