...
export default function UserProfileRoleCard({ role }: { role: UserRole }) {
  ...
  return (
    <section className="rounded-[1.75rem] border border-black/5 bg-white p-5 shadow-sm">
      ...
      <p className="mt-5 text-xs font-semibold uppercase tracking-[0.18em] text-zinc-400">{t("profile.roleLabel")}</p>
      <div className={`mt-2 inline-flex rounded-full border px-3 py-1.5 text-sm font-bold ${styles.badge}`}>{t(copy.labelKey)}</div>
      <p className="mt-3 text-sm leading-6 text-zinc-500">{t(copy.descriptionKey)}</p>
    </section>
  );
}
