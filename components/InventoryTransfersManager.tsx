    ...
    <div className="space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      {scanResult ? (
        <section className="rounded-2xl border border-emerald-200 bg-white p-5 shadow-sm" aria-label="Результат сканирования">
          ...
          <div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => void requestTransfer()} disabled={busy} className="rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-50">Запросить передачу</button><button type="button" onClick={() => setScanResult(null)} className="rounded-xl border border-zinc-200 px-4 py-2.5 text-sm font-semibold text-zinc-700">Отмена</button></div>
        </section>
      ) : null}
