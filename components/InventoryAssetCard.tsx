...
export default function InventoryAssetCard({ item, canGenerateQr }: { item: InventoryItem; canGenerateQr: boolean }) {
  ...
  const category = isInventoryItemCategory(item.category)
    ? t(inventoryItemCategoryTranslationKey(item.category))
    : item.category;
  return (
