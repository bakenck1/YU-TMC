...
test("a free active QR item offers receiving it for the current employee", () => {
  assert.deepEqual(
    employeeScanAction({ status: "active", responsibleName: null, isCurrentUserResponsible: false }),
    { kind: "claim_free" },
  );
});
...
test("an item assigned to another employee cannot be claimed and offers a transfer request", () => {
  assert.deepEqual(
    ...
    { kind: "request_transfer", ownerName: "Айдана Нурова" },
  );
});

test("an employee's own QR item is shown as already assigned to them", () => {
  assert.deepEqual(
    employeeScanAction({ status: "active", responsibleName: "Сотрудник", isCurrentUserResponsible: true }),
    { kind: "already_owned" },
  );
});
...
test("inactive QR items cannot be received or requested", () => {
  assert.deepEqual(
    employeeScanAction({ status: "maintenance", responsibleName: null, isCurrentUserResponsible: false }),
    { kind: "unavailable" },
  );
});
